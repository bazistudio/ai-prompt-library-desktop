...
import { Sparkles, ArrowRight, X, RefreshCw, CheckCircle2, CheckCircle, AlertCircle } from "lucide-react";
...
export const UpdateNotificationModal: React.FC<UpdateNotificationModalProps> = ({ updater }) => {
  ...
  const isUpToDate = updater.status === "up-to-date" && !updater.availableVersion;
  const isDownloaded = updater.status === "downloaded" && !!updater.availableVersion;

  return (
    <div
      className="fixed bottom-6 right-6 z-50 max-w-md w-full p-5 rounded-2xl bg-card/95 backdrop-blur-xl border border-primary/30 shadow-2xl shadow-primary/10 text-foreground animate-in slide-in-from-bottom-5 duration-300 select-none"
      role="dialog"
    ...
    >
          ...
          <div>
            <h3 id="update-modal-title" className="text-sm font-bold tracking-tight text-foreground">
              {isChecking
                ...
                : isDownloading
                ? `Downloading Update (${updater.downloadProgress}%)...`
                : isUpToDate
                ? "You're Up to Date"
                : isDownloaded
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {updater.manualMessage ||
                (isChecking
                  ? "Contacting GitHub Releases repository..."
                  : isDownloading
                  ? `Downloading v${updater.availableVersion} in the background.`
                  : isUpToDate
                  ? `AI Prompt Library v${updater.currentVersion} is the latest version.`
                  : isDownloaded
                  ? `AI Prompt Library v${updater.availableVersion} is downloaded and verified.`
                  : `Currently installed version: v${updater.currentVersion}`)}
            </p>
          </div>
        </div>
      ...
      {/* Version Comparison Card (when update is available or downloaded) */}
      ...
      {isDownloading && (
        <div className="mt-3 space-y-1">
          <div className="flex justify-between text-[10px] text-muted-foreground font-medium">
            <span className="text-primary font-semibold">Downloading package...</span>
            <span className="font-mono">{updater.downloadProgress}%</span>
          </div>
      ...
      {updater.releaseNotes && isDownloaded && (
      ...
      <div className="mt-4 flex items-center justify-end gap-2.5">
        {isUpToDate && (
          <button
            type="button"
            onClick={updater.dismissModal}
            className="px-4 py-2 rounded-xl text-xs font-semibold bg-primary text-primary-foreground hover:bg-primary/90 transition-all cursor-pointer shadow-sm"
          >
            OK
          </button>
        )}
